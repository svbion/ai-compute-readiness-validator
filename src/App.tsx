...
function EngagementDetailPage({ engagementId }: { engagementId: string }) {
  ...
  const refreshEngagement = () => {
    ...
    ]).then(([engagementPayload, nodesPayload, evidencePayload, activityPayload, comparisonPayload, findingsPayload, readinessPayload]) => {
    ...
    <EngagementShell>
      ...
      <section className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
        <div className="space-y-6">
          ...
          <Panel title="Benchmarks"><div className="mb-3 text-sm text-slate-400">Benchmark results are evaluated separately and are not included in the current readiness score.</div><div className="grid gap-3 md:grid-cols-3">{["NCCL", "HPL", "Inference"].map((name) => <div key={name} className="rounded-2xl border border-slate-800 bg-slate-950/40 p-4"><div className="font-semibold text-slate-100">{name}</div><div className="mt-2 text-xs font-mono uppercase tracking-wider text-amber-300">Awaiting Benchmark Evidence</div></div>)}</div></Panel>
        </div>
        <div className="space-y-6">
