...
export default function App() {
  ...
  const triggerScenarioRun = async () => {
    setLoading(true);
    setError(null);
    setLoadingMessage(`Running ${scenarioMetadata[selectedScenario].label.toLowerCase()} through the validation CLI...`);
    setCluster(null);
    setExpandedChecks({});
    ...
    try {
      ...
      if (!response.ok) {
        const payload = await response.json().catch(() => ({ error: "Failed to run validation scenario." }));
        throw new Error(payload.error ?? "Failed to run validation scenario.");
      }
      ...
      setError(runError instanceof Error ? runError.message : "Failed to run validation scenario.");
    } finally {
      setLoading(false);
    }
  };
            ...
            <div className="xl:min-w-[360px] space-y-3">
              <div className="cyber-panel rounded-2xl border border-slate-800/80 p-3.5">
                ...
                <div className="mt-3 grid gap-3 sm:grid-cols-[1fr_auto] sm:items-center">
                  ...
                  <button
                    onClick={triggerScenarioRun}
                    disabled={loading}
                    className="inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-500 px-4 py-2.5 text-sm font-semibold text-slate-950 transition hover:bg-emerald-400 disabled:cursor-not-allowed disabled:bg-emerald-700"
                  >
                    {loading ? <RefreshCw className="h-4 w-4 animate-spin" /> : <Play className="h-4 w-4" />}
                    {loading ? "Running scenario" : "Run validation scenario"}
                  </button>
                </div>
              </div>
