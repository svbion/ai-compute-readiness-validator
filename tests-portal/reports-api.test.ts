...
test("report preview route renders and persists professional HTML with source lineage", async () => {
  await withServer(async (baseUrl, storePath, reportsDir) => {
    ...
    const generated = await jsonRequest(baseUrl, "POST", `/api/v1/reports/${reportId}/generate/html`);
...
test("reports API generates HTML preview through explicit generation endpoint before direct preview access", async () => {
  await withServer(async (baseUrl, storePath, reportsDir) => {
    const { agentId, validationId } = await seedLiveA100Validation(baseUrl);
    const created = await jsonRequest(baseUrl, "POST", "/api/v1/reports", {
      name: "RunPod A100 Generated HTML Preview",
      report_type: "customer-validation",
      status: "generating",
      scope_type: "validation_run",
      scope_id: validationId,
      ...
      benchmark_ids: ["bmk_selected"],
      agent_ids: [agentId],
      validation_ids: [validationId],
      include_evidence: true,
    });
    assert.equal(created.response.status, 201, JSON.stringify(created.payload));
    const reportId = created.payload.report.report_id;

    const generatingPreview = await fetch(`${baseUrl}/portal/reports/${reportId}/preview`, { headers: authHeader });
    const generatingHtml = await generatingPreview.text();
    assert.equal(generatingPreview.status, 202);
    assert.match(generatingHtml, /Report still generating/);

    const generated = await jsonRequest(baseUrl, "POST", `/api/v1/reports/${reportId}/generate/html`);
    assert.equal(generated.response.status, 200, JSON.stringify(generated.payload));
    assert.equal(generated.payload.report.report_id, reportId);
    assert.equal(generated.payload.report.status, "generated");
    assert.match(generated.payload.report.html_artifact_path, new RegExp(`${reportId}\\.html$`));
    assert.ok(generated.payload.report.html_artifact_path.startsWith(reportsDir));
    assert.match(generated.payload.report.html_sha256, /^[a-f0-9]{64}$/);
    assert.ok(Date.parse(generated.payload.report.html_generated_at));
    ...
    assert.equal(generated.payload.preview.url, `/portal/reports/${reportId}/preview`);

    const htmlPath = generated.payload.report.html_artifact_path;
    assert.ok(fs.existsSync(htmlPath));
    const persistedHtml = fs.readFileSync(htmlPath, "utf8");
    for (const expected of ["Generated by Sabion P Frazier", "Purpose: GPUValidator interview demonstration", "Not collected", "Not available", "Not supported", "Validation not run"]) {
      assert.match(persistedHtml, new RegExp(expected.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")), `${expected} should be present in generated preview`);
    }

    const direct = await fetch(`${baseUrl}/portal/reports/${reportId}/preview`, { headers: authHeader });
    const directHtml = await direct.text();
    assert.equal(direct.status, 200, directHtml);
    assert.equal(directHtml, persistedHtml);

    const storeDoc = JSON.parse(fs.readFileSync(storePath, "utf8"));
    const persisted = storeDoc.reports.find((report: any) => report.report_id === reportId);
    assert.equal(persisted.html_artifact_path, htmlPath);
    assert.equal(persisted.status, "generated");
  });
});

test("report preview route handles not found and failed generation states as browser-visible pages", async () => {
  await withServer(async (baseUrl) => {
    const missing = await fetch(`${baseUrl}/portal/reports/rpt_missing/preview`, { headers: authHeader });
    const missingHtml = await missing.text();
    assert.equal(missing.status, 404);
    assert.match(missingHtml, /Report not found/);

    const created = await jsonRequest(baseUrl, "POST", "/api/v1/reports", {
      name: "Failed preview state",
      report_type: "management-status",
      status: "failed",
      scope_type: "cluster",
      scope_id: "cluster-diag",
      error: "Renderer failed without secrets",
    });
    assert.equal(created.response.status, 201, JSON.stringify(created.payload));
    const failed = await fetch(`${baseUrl}/portal/reports/${created.payload.report.report_id}/preview`, { headers: authHeader });
    const failedHtml = await failed.text();
    assert.equal(failed.status, 500);
    assert.match(failedHtml, /Report generation failed/);
    assert.match(failedHtml, /Renderer failed without secrets/);
  });
});
...
test("reports API generates persists and downloads a valid DOCX archive with expected report text", async () => {
  await withServer(async (baseUrl, storePath, reportsDir) => {
    const { agentId, validationId } = await seedLiveA100Validation(baseUrl);
    const created = await jsonRequest(baseUrl, "POST", "/api/v1/reports", {
      name: "RunPod A100 DOCX Export",
      report_type: "customer-validation",
      scope_type: "validation_run",
      scope_id: validationId,
      customer: "NVIDIA interview demo",
    ...
    const generated = await jsonRequest(baseUrl, "POST", `/api/v1/reports/${reportId}/generate/docx`);
    assert.equal(generated.response.status, 200, JSON.stringify(generated.payload));
    assert.equal(generated.payload.report.docx_mime_type, "application/vnd.openxmlformats-officedocument.wordprocessingml.document");
    assert.match(generated.payload.report.docx_artifact_path, /gpuvalidator_customer-validation_cluster_runpod_a100_\d{8}_v4\.docx$/);
    assert.ok(generated.payload.report.docx_artifact_path.startsWith(reportsDir));
    assert.ok(generated.payload.report.docx_size_bytes > 1000);
    assert.match(generated.payload.report.docx_sha256, /^[a-f0-9]{64}$/);
    assert.ok(Date.parse(generated.payload.report.docx_generated_at));
    assert.equal(generated.payload.report.docx_template_version, "1.0.0");
    assert.equal(generated.payload.report.error, null);
    assert.deepEqual(generated.payload.report.validation_ids, [validationId]);
    assert.deepEqual(generated.payload.report.agent_ids, [agentId]);

    const docxPath = generated.payload.report.docx_artifact_path;
    assert.ok(fs.existsSync(docxPath));
    const docx = fs.readFileSync(docxPath);
    assert.equal(docx.subarray(0, 2).toString("ascii"), "PK");
    assert.equal(crypto.createHash("sha256").update(docx).digest("hex"), generated.payload.report.docx_sha256);
    assert.equal(docx.length, generated.payload.report.docx_size_bytes);
    const archiveText = docx.toString("utf8");
    for (const expected of [
      "[Content_Types].xml",
      "word/document.xml",
      "word/header1.xml",
      "word/footer1.xml",
      "RunPod A100 DOCX Export",
      ...
      assert.match(archiveText, new RegExp(expected.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")), `${expected} should be present in DOCX archive`);
    }

    const downloaded = await fetch(`${baseUrl}/api/v1/reports/${reportId}/download/docx`, { headers: authHeader });
    const downloadedDocx = Buffer.from(await downloaded.arrayBuffer());
    assert.equal(downloaded.status, 200);
    assert.match(downloaded.headers.get("content-type") ?? "", /application\/vnd\.openxmlformats-officedocument\.wordprocessingml\.document/);
    assert.match(downloaded.headers.get("content-disposition") ?? "", /attachment; filename="gpuvalidator_customer-validation_cluster_runpod_a100_\d{8}_v4\.docx"/);
    assert.equal(downloadedDocx.subarray(0, 2).toString("ascii"), "PK");
    assert.equal(downloadedDocx.length, docx.length);

    const storeDoc = JSON.parse(fs.readFileSync(storePath, "utf8"));
    const persisted = storeDoc.reports.find((report: any) => report.report_id === reportId);
    assert.equal(persisted.docx_artifact_path, docxPath);
    assert.equal(persisted.docx_mime_type, "application/vnd.openxmlformats-officedocument.wordprocessingml.document");
    assert.equal(persisted.docx_size_bytes, docx.length);
    assert.equal(persisted.docx_sha256, generated.payload.report.docx_sha256);
    assert.ok(Date.parse(persisted.docx_generated_at));
    assert.equal(persisted.docx_template_version, "1.0.0");
  });
});

test("reports API returns detailed DOCX diagnostics when download cannot complete", async () => {
  await withServer(async (baseUrl) => {
    const created = await jsonRequest(baseUrl, "POST", "/api/v1/reports", {
      name: "DOCX diagnostics",
      report_type: "management-status",
      scope_type: "cluster",
      scope_id: "cluster-diag",
    });
    ...
    const missingDownload = await fetch(`${baseUrl}/api/v1/reports/${reportId}/download/docx`, { headers: authHeader });
    const missingPayload = await missingDownload.json();
    assert.equal(missingDownload.status, 409);
    assert.equal(missingPayload.error.code, "docx_not_generated");
    assert.match(missingPayload.error.message, /Generate the DOCX/);
    assert.deepEqual(missingPayload.error.details.map((item: any) => item.field), ["docx_artifact_path"]);
  });
});
